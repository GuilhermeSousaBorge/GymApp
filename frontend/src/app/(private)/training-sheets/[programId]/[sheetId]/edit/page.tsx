...
const EditTrainingSheetPage = () => {
    ...
    const { data: sheet, isLoading, error } = useSheetDetails(isEditing ? Number(params.sheetId) : undefined)
    const { mutateAsync: createSheet } = useCreateSheet()
    const { mutateAsync: updateSheet } = useUpdateSheet()

    const { register, handleSubmit, control, formState: { errors, isSubmitting } } = useForm<TrainingSheetFormData>({
        resolver: zodResolver(trainingSheetSchema),
        values: {
            active: sheet?.active ?? true,
            description: sheet?.description ?? "",
            name: sheet?.name ?? "",
            trainingProgramId: sheet?.trainingProgramId ?? Number(params.programId),
    ...
    const onSubmit = async (data: TrainingSheetFormData) => {
        try {
            if (isEditing) {
                await updateSheet({ id: Number(params.sheetId), sheet: data })
            ...
            router.push(`/training-sheets/${params.programId}`)
        } catch (err) {
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="flex items-center gap-4">
                <Button
                    ...
                    variant="outline"
                    size="icon"
                    onClick={() => router.push(`/training-sheets/${params.programId}`)}
                >
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                            {isEditing ? "Atualize as informações da ficha" : "Preencha as informações da nova ficha"}
                        </p>
                    </div>
                </div>
            </div>
